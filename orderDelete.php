...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    try {
        ...
        if ($result->num_rows > 0) {
            ...
            echo "<script>alert('Order and associated items deleted successfully.'); window.location.href='order.php';</script>";
        } else {
        ...
        $conn->rollback();
        echo "<script>alert('" . $e->getMessage() . "'); window.location.href='order.php';</script>";
    }

    // Close the connection
    $conn->close();
} else {
    echo "<script>alert('No order ID specified.'); window.location.href='order.php';</script>";
}
?>
